# RAM — Integration Design Document (High Level)
...
| **Related model** | [`architecture/tobe/ram-architecture.c4`](../../architecture/tobe/ram-architecture.c4) (LikeC4 source) |
...
_[Placeholder: distribution list]_ — e.g. RAM design & delivery team; owning teams of the integrating systems (JOH Source System (eLinks), MRD Source, Payment System, client systems); technical governance board.

---

## Background — what RAM is (product view)

**RAM (Resource Allocation Manager)** is the system used to plan and manage **when and where judicial office holders (JOHs) sit**. Its product goal is to ensure the right judicial resource is available at the right venue on the right day, across jurisdictions (**Tribunals**, **Courts**, and — in a future stage — **Magistrates**).

From a product perspective, RAM:
...
Confirming a sitting in RAM is the event that enables a JOH to be **paid**. RAM is delivered in stages, beginning with **SSCS**, the first Tribunal sub-jurisdiction (see §2).

---

## 1. Purpose & scope

This document describes, **at a high level**, the integrations between RAM and the external systems and user groups it exchanges data with. It is intended for **technical governance and the owning teams of the integrating systems**, to establish a shared understanding of:

- which systems exchange data with RAM, and in which direction;
- the mechanism, ownership and current delivery phase of each integration;
- the responsibilities of each owning team.

**Direction is stated by which end RAM is on:**

- **Inbound** — RAM is on the receiving end: it ingests data from a source system, *or* it hosts the API that an external party calls into.
- **Outbound** — RAM is on the sending end: it produces data that is sent out to an external system.
- **Interactive** — users acting through the RAM user interface.

> Client systems are classified **inbound** even though they *consume* RAM data, because they call into a RAM-hosted API — RAM is on the receiving end of the request. The availability data RAM returns flows back outward in the response.

**In scope:** the system-to-system data feeds and the primary user/UI interaction shown in the RAM system context.

**Out of scope:** internal RAM component design, detailed data contracts, field-level mappings (see [References](#8-references)), and non-functional sizing — these are captured in dedicated artefacts or marked as TODO below.
...
RAM is delivered in stages, **Tribunals first**. Within Tribunals, the sub-jurisdictions are delivered in the order **SSCS → ET → IAC**. **Phase 1 delivers SSCS** — the first Tribunal sub-jurisdiction. Integration availability and cadence may differ by phase / jurisdiction.
...
The high-level system context below is the primary overview of RAM's external integrations and the source this document elaborates on (currently at draft revision v0.1D). It is generated from the LikeC4 model and should be regenerated when that model changes (see [References](#8-references)).

![RAM System Context](../../architecture/tobe/diagrams/systemContext.png)

*Figure 1 — RAM System Context (v0.1D Draft). Source: `architecture/tobe/diagrams/systemContext.png`.*

For the internal container/component view that these integrations connect into, see [`ramContainers.png`](../../architecture/tobe/diagrams/ramContainers.png).
...
| 3 | **Client systems** | Inbound | Consumes JOH availability (queries RAM) | API (read-only, client-initiated) | _[Placeholder: phase]_ |
| 4 | **Payment System** | Outbound | Consumes payment extract | Manual upload | Phase 1 (SSCS) — manual |
| 5 | **Users ↔ RAM UI** | Interactive | Manage sittings / view data | Web UI | _[Placeholder: phase]_ |
...
Supplies judicial office holder (JOH) personnel information. RAM's **JOH Data Scheduler** fetches data via API and persists the raw payload to the **JOH Raw Payload Store**; the **JOH Data Reader** then reads the raw payload and sends personnel information to the **JOH Management API**, which persists it to the RAM database.

| Attribute | Value |
|---|---|
| Direction | Inbound (source → RAM) |
| Source → Target | JOH Source System (eLinks) → RAM (JOH Data Scheduler → JOH Raw Payload Store → JOH Data Reader → JOH Management API → RAM Database) |
...
Supplies the **MRD dataset**. The data is delivered to RAM via an **off-system process** (i.e. not a live system-to-system interface in Phase 1). RAM's **MRD Data Consumer** reads the delivered dataset and persists it to the RAM database.

| Attribute | Value |
|---|---|
| Direction | Inbound (source → RAM) |
| Source → Target | MRD Source → RAM (MRD Data Consumer → RAM Database) |
...
| Detailed design | _[Placeholder: link once `docs/integrations/mrd/` is populated]_ |
| Phase / status | _[Placeholder]_ |

> **TODO:** Expand "MRD" on first use and describe the dataset's purpose within RAM. Source inputs are under `input-docs/mrd/`.

### 5.3 Client systems — Inbound

External client systems query RAM for **JOH availability** — for example, to answer "which JOH is sitting at which location on a given day". Although these systems **consume** RAM data, the integration is **inbound** to RAM: the client initiates a read-only call into RAM's **Availability API**, and RAM returns availability data (read from the RAM database) in the response.

| Attribute | Value |
|---|---|
| Direction | Inbound (client-initiated query; data returned by RAM in response) |
| Source → Target | Client systems → RAM Availability API → RAM Database |
...
RAM produces a payment data extract that is consumed by the external **Payment System**. In the current phase this is a **manual** process: **Finance Users** extract payment data from RAM and upload it to the Payment System.

| Attribute | Value |
|---|---|
| Direction | Outbound (RAM → Payment System) |
| Source → Target | RAM → (Finance Users, manual) → Payment System |
...
### 5.5 Users ↔ RAM UI — Interactive

User groups interact with RAM through the **RAM User Interface**, which calls the internal RAM APIs (Sitting Management, Absence Management, JOH Management, Authorisation Management). At login, the **Authorisation Management API** resolves the user's jurisdiction(s) and user type, which drives what data and lookups are visible.

| Attribute | Value |
|---|---|
| Direction | Interactive (users ↔ RAM UI) |
...
| Mechanism | Web UI over internal RAM APIs |
...
| Detailed design | — (interactive UI; see the RAM Capabilities and RAM Containers diagrams) |
| Phase / status | _[Placeholder]_ |
...
| Artefact | Location |
|---|---|
| LikeC4 architecture model (source of the diagrams) | [`architecture/tobe/ram-architecture.c4`](../../architecture/tobe/ram-architecture.c4) |
| System context diagram | [`architecture/tobe/diagrams/systemContext.png`](../../architecture/tobe/diagrams/systemContext.png) |
| RAM containers diagram | [`architecture/tobe/diagrams/ramContainers.png`](../../architecture/tobe/diagrams/ramContainers.png) |
| Capability map | [`architecture/tobe/diagrams/capabilityMap.png`](../../architecture/tobe/diagrams/capabilityMap.png) |
| JOH Source System (eLinks) — ER diagram | [`docs/integrations/jo/jo-er-diagram.md`](./jo/jo-er-diagram.md) |
| JOH Source System (eLinks) — schema mapping | [`docs/integrations/jo/jo-schema-mapping.md`](./jo/jo-schema-mapping.md) |
| MRD integration detail | _[Placeholder: `docs/integrations/mrd/`]_ |
...
| **Tribunals / Courts / Magistrates** | The judicial jurisdictions RAM serves (Magistrates TBD) |
